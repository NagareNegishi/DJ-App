...
{
    dir.getChildFile (fileName).replaceWithText (
        "this is not audio data, just garbage text pretending to be a wav file");
}
...
TEST_CASE ("LocalFileRepository rejects manifest entries whose id fails the id pattern",
           "[repository]")
{
    SECTION ("empty id")
    {
        ...
        writeManifest (fx.dir,
            R"({ "tracks": [ { "id": "", "title": "T", "file": "track.wav" } ] })");
    ...
    SECTION ("id containing a space")
    {
        ...
        writeTestWav (fx.dir, "track.wav", 1, 4410, 44100.0);
        writeManifest (fx.dir,
            R"({ "tracks": [ { "id": "bad id", "title": "T", "file": "track.wav" } ] })");
...
TEST_CASE ("LocalFileRepository rejects manifest entries with unsafe file paths",
           "[repository]")
{
    SECTION ("file containing '..'")
    {
        ...
        writeManifest (fx.dir,
            R"({ "tracks": [ { "id": "bad1", "title": "T", "file": "../)"
                + outsideDir.getFileName() + "/" + sibling.getFileName() +
                R"(" } ] })");
    ...
    SECTION ("file containing a forward slash (subdirectory)")
    {
        ...
        writeManifest (fx.dir,
            R"({ "tracks": [ { "id": "bad2", "title": "T", "file": "sub/track.wav" } ] })");
    ...
    SECTION ("file containing a backslash")
    {
        ...
        writeManifest (fx.dir,
            R"({ "tracks": [ { "id": "bad3", "title": "T", "file": "a\\b.wav" } ] })");
    ...
    SECTION ("file given as an absolute path")
    {
        TempDirFixture fx;
        writeManifest (fx.dir,
            R"({ "tracks": [ { "id": "bad4", "title": "T", "file": "/etc/passwd" } ] })");
...
TEST_CASE ("LocalFileRepository rejects a manifest entry whose file is a symlink",
           "[repository]")
{
    TempDirFixture fx;
    ...
    writeManifest (fx.dir,
        R"({ "tracks": [ { "id": "symlinked", "title": "T", "file": "linked.wav" } ] })");
...
TEST_CASE ("LocalFileRepository rejects a manifest entry whose file does not exist on disk",
           "[repository]")
{
    TempDirFixture fx;
    writeManifest (fx.dir,
        R"({ "tracks": [ { "id": "missing1", "title": "T", "file": "nope.wav" } ] })");
...
TEST_CASE ("LocalFileRepository getAudioBuffer never crashes on an undecodable file",
           "[repository]")
{
    TempDirFixture fx;
    writeGarbageFile (fx.dir, "garbage.wav");
    writeManifest (fx.dir,
        R"({ "tracks": [ { "id": "garbage1", "title": "T", "file": "garbage.wav" } ] })");
...
TEST_CASE ("LocalFileRepository handles a missing or unparseable manifest without crashing",
           "[repository]")
{
