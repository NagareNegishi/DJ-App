...
- **Follow-up (2026-08-24): the unused `signalsmith_linear` `FetchContent` pin was removed** from `client/CMakeLists.txt` (it was never linked into anything, per the point above - confirmed genuinely dead, not just unreferenced by one call site). The §5/§6.1 text above still describes the original two-dependency design-gate decision as made at the time; this note and `docs/plan/DEVIATIONS.md`'s 2026-08-22 entry (which first established the no-dependency finding) are the record of why only one of the two pins survived integration. `THIRD_PARTY_NOTICES.md` accordingly lists Signalsmith Stretch only.
